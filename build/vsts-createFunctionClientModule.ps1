...
$parentModule = 'CertificateExpiration'
if (-not $ModuleName) { $ModuleName = 'CertificateExpiration.Client' }
...
$publishRoot = Join-Path -Path $WorkingDirectory -ChildPath 'publish\CertificateExpiration'
...
$originalManifestData = Import-PowerShellDataFile -Path "$publishRoot\CertificateExpiration.psd1"
$prereqHash = @{
