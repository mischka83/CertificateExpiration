...
#>
	...
	process
	{
		try { $uri = '{0}{1}' -f (Get-PSFConfigValue -FullName 'CertificateExpiration.Client.Uri' -NotNull), $FunctionName }
		catch { $PSCmdlet.ThrowTerminatingError($_) }
		$header = @{ }
		
		#region Authentication
		$unprotectedToken = Get-PSFConfigValue -FullName 'CertificateExpiration.Client.UnprotectedToken'
		$protectedToken = Get-PSFConfigValue -FullName 'CertificateExpiration.Client.ProtectedToken'
